import { productModel } from "../content/kithContent";
import SectionHeading from "../components/SectionHeading";

export default function ProductModel() {
  return (
    <section id="model" className="border-b border-ledger-line py-16 sm:py-20">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        {/* in-code domain diagram — three primary nodes, connected, with satellite concerns below */}
        <div className="mt-12">
          ...
          {/* concrete instance — the same three nodes, but with real data,
              so the containment relationship reads immediately rather than
              staying abstract */}
          <div className="mt-8 border border-ledger-line bg-white p-6 sm:p-7">
            <p className="eyebrow mb-4">What that looks like with real data</p>
            <pre className="overflow-x-auto font-mono text-xs leading-loose text-ink/80 sm:text-sm">
{`Workspace   `}<span className="text-ink">The Adeyemi Family</span>{`
  └── Container   `}<span className="text-ink">Mom's 60th Birthday</span>{`
        └── Ledger Entry   `}<span className="text-confirmed">₦50,000 — confirmed</span>
            </pre>
          </div>
        </div>
      </div>
